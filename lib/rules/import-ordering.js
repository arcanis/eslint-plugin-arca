...
module.exports = {
    ...
    create: function(context) {
        ...
        var importDeclarations = sourceCode.ast.body.filter(function(node) {
        ...
        function reportExpectedModuleToBeImportedBeforeAnotherError(node, source, otherSource, reason) {
            ...
            return context.report({
                ...
                fix: function(fixer) {
                    var fromRange = importDeclarations[0].range[0];
                    var toRange = importDeclarations[importDeclarations.length - 1].range[1];
                    var suggestedFix = sortAndFixAllNodes(importDeclarations);

                    return suggestedFix
                        ? fixer.replaceTextRange([fromRange, toRange], suggestedFix)
                        : null;
                }
            });
        ...
        function sortAndFixAllNodes(nodes) {
            var imports = nodes.slice();

            for (var i = 0; i < imports.length; i++)
                if (sourceCode.getCommentsBefore(imports[i]).length || sourceCode.getCommentsAfter(imports[i]).length)
                  // If there are comments don't rearrange
                  return null;

            var sorted = imports.sort(function(a, b) {
                return compareModules(a, b) === null ? -1 : 1
            });

            return sorted.reduce(function(sourceText, declaration, index) {
                var textAfterSpecifier = index === importDeclarations.length - 1
                    ? ""
                    : sourceCode.getText().slice(importDeclarations[index].range[1], importDeclarations[index + 1].range[0]);
